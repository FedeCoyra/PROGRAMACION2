...
char* p_ej4_cambiarbase(int nrobasedecimal, int nrootrabase) {
    ...
    while ((numeroAConvertir > 0) && (contadorParaMax <= TAMANIO_MAXIMO)) {
        int resto = numeroAConvertir % nrootrabase;
        TipoElemento elemento = te_crear(resto);
        p_apilar(pila, elemento);
        numeroAConvertir = numeroAConvertir / nrootrabase;
        contadorParaMax++;
        if (contadorParaMax > TAMANIO_MAXIMO) {
            printf("Debido a que la cant. de digitos del numero resultante es mas grande que el tamanio maximo de la pila, el numero quedara incompleto. \n");
        }
    }
...
int main() {
    ...
    int base = 0;
    printf("Ingrese la base a la que desea convertir el base (2 - 16): ");
    base = leer_entero();
