...
Pila llenarPila(Pila pila) {
    ...
    int longitud = 0;
    printf("Ingrese la longitud de la pila (tamano de 1 a %d): ", TAMANIO_MAXIMO);
    longitud = leer_entero();

    while (longitud < 1 || longitud > TAMANIO_MAXIMO) {
        printf("Entrada invalida. Se supero el rango disponible para la pila. Reingrese el tamanio. ");
        longitud = leer_entero();
    }
