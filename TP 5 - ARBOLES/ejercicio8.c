...
void calcularNivel(NodoArbol nodo, int actual, int nodoA, int * nivel) {
    if (!a_es_rama_nula(nodo)) {
        ...
        calcularNivel(n_hijoderecho(nodo), actual, nodoA, nivel);
    }
}
...
void calcularNodosInternos (NodoArbol nodo, Lista lista) {
    if (!a_es_rama_nula(nodo)) {
        if (!a_es_rama_nula(n_hijoizquierdo(nodo))) {
            l_agregar(lista, n_recuperar(nodo));
        }
        calcularNodosInternos(n_hijoizquierdo(nodo), lista);
        calcularNodosInternos(n_hijoderecho(nodo), lista);
    }
}

Lista c_ej8_internos(ArbolBinario A) {
    Lista lista = l_crear();
    calcularNodosInternos(n_hijoizquierdo(a_raiz(A)), lista);
    return lista;
}
...
void hojasAlMismoNivel(NodoArbol nodo, int *nivel, int nivelActual, bool *mismo) {
    if (!a_es_rama_nula(nodo)) {
        ...
        hojasAlMismoNivel(n_hijoderecho(nodo), nivel, nivelActual, mismo);
    }
}
...
bool c_ej8_hojasmismonivel(ArbolBinario A) {
    ...
    int nivel = -1;
    hojasAlMismoNivel(n_hijoizquierdo(a_raiz(A)), &nivel, 1, &mismoNivel);
    return mismoNivel;
}
