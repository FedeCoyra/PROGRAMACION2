...
static const int TAMANIO_MAXIMO_COLAS = 2000;
...
bool c_encolar(Cola cola, TipoElemento elemento) { // O(n)
    if (c_es_llena(cola)) {
        return false;
    }
...
//-----------------------------------------------------------
int longitudCola(Cola cola) { // O(n)
    int i = 0;
