...
Cola rellenar_colas_por_teclado_sin_repetidos() {
    Cola cola = c_crear();
    int longitudCola = 0;
    printf("Ingrese la longitud de la cola (maximo %d elementos). ", TAMANIO_MAXIMO_COLAS);
    longitudCola = leer_entero();
    while (longitudCola > TAMANIO_MAXIMO_COLAS || longitudCola <= 0) {
...
int main() {
    ...
    if (divisor != 0) {
        if (fuetotal) {
            ...
            printf("Al ser un divisor total, tiene prioridad, por ende no se mostraran divisores parciales (en caso de existir).\n");
        } else {
