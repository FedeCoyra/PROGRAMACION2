...
struct IteradorRep {
    struct NodoLista *posicionActual;
};
...
bool l_agregar(Lista lista, TipoElemento elemento) { // O(n)
    ...
    // Ahora lo agrego
    struct NodoLista *nuevo_nodo = malloc(sizeof(struct NodoLista));
    ...
    if (lista->inicio == NULL) {
        ...
        struct NodoLista *temp2 = lista->inicio;
...
bool l_borrar(Lista lista, int clave) { // O(n)
    ...
    while (actual != NULL && actual->siguiente != NULL) {
        if (actual->siguiente->datos->clave == clave) {
            struct NodoLista *temp = actual->siguiente;
...
bool l_eliminar(Lista lista, int pos) { // O(n)
    ...
    if (1 <= pos && pos <= l_longitud(lista)) {
        if (pos == 1) {
            ...
            struct NodoLista *temp = actual->siguiente; // nodo en pos
